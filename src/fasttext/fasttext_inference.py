...
class FasTextPipeline:
    ...
    def __call__(self, company_1: str, company_2: str = None, top_n: int = 10):
        if company_2  is not None:
            ...
            ft_company_vectors["score"] = ft_company_vectors.apply(lambda x: spatial.distance.euclidean(company_1, x["vectors"]), axis = 1)
            return ft_company_vectors.drop_duplicates(subset=["score"]).sort_values(by=["score"])[0:top_n]["senteses"].tolist()
